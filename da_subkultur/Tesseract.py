...
def highlight_image(img, xml):
    ...
    image.save(img + "_col.jpg")    # Die einzelne Seite vom Artikel mit den gekennzeichneten Bilder wird lokal abgespeichert
...
def get_Text(img, xml):
    ...
    for t in elementpath.select(root, '//String', {'': 'http://www.loc.gov/standards/alto/ns-v3#'}):
        ...
        if done == False:   # Solange done False ist
...
def crop_img(img, shape, anz, dir):
    ...
    if im1.width == 0 or im1.height == 0:       #Dann wird geprüft ob das Bild überhaupt eine Höhe und Breite hat
        ...
        im1.save(dir + "\\\\" + name +"_pic_"+str(anz)+".jpg")  #Das asugeschnittene Bilder wird im entsprechenden ordner gespeichert
